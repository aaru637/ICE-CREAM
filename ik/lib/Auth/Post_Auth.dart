...
class Post_Auth {
  ...
  static Future<String> uploadImageData(
      AssetEntity file, String postType, String pid) async {
    Uint8List? image = await file.thumbnailData;
    final reference = storage
        .ref()
        .child('posts')
        .child(postType)
  ...
  static Future<void> uploadVideoPost(
      AssetEntity file, String postType, BuildContext context) async {
    try {
      File f = File.fromUri(Uri.file('${file.relativePath}/${file.title}'));
      String pid = const Uuid().v1();
      var reference = storage
          ...
          .child(UserProvider.getUser()!.id)
          .child(pid);
      uploadTask = reference.putFile(f);
      final snap = await uploadTask;
